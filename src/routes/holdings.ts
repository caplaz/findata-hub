  ...
  try {
    ...
    try {
      result = await yahooFinance.quoteSummary(symbol, {
        modules: ["topHoldings", "sectorWeightings", "equityHoldings"] as any,
      });
    } catch (holdingsError) {
      log(
        "warn",
        `Detailed holdings not available for ${symbol}, using fallback: ${
          (holdingsError as Error).message
        }`
      );
 ...
 *               $ref: '#/components/schemas/Error'
 */
router.get("/:symbol/fund", async (req: Request, res: Response) => {
  const symbol = req.params.symbol.toUpperCase();
  ...
  try {
    const result = await yahooFinance.quoteSummary(symbol, {
      modules: ["fundHoldings", "fundProfile"] as any,
    });
